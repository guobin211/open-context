...
use crate::app_state::{DatabaseManager, Note, NoteType};
...
impl DatabaseManager {
    ...
    pub fn create_note(&self, note: &Note) -> SqliteResult<()> {
        ...
        conn.execute(
            "INSERT INTO notes (id, workspace_id, title, note_type, content, file_path, tags, created_at, updated_at)
    ...
    pub fn get_note(&self, id: &str) -> SqliteResult<Option<Note>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, workspace_id, title, note_type, content, file_path, tags, created_at, updated_at
    ...
    pub fn update_note(&self, note: &Note) -> SqliteResult<()> {
        ...
        conn.execute(
            "UPDATE notes
             SET title = ?1, note_type = ?2, content = ?3, file_path = ?4, tags = ?5, updated_at = ?6
             WHERE id = ?7",
            params![
                note.title,
                note.note_type.as_str(),
