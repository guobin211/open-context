...
export async function upsertSymbol(db: Surreal, symbol: SurrealSymbol): Promise<void> {
  try {
    const recordId = `symbol:${symbol.symbol_id}`;
    // @ts-ignore
    await db.upsert(recordId, symbol as unknown as Record<string, unknown>);
  } catch (error) {
...
export async function batchUpsertSymbols(db: Surreal, symbols: SurrealSymbol[]): Promise<void> {
  try {
    const promises = symbols.map((symbol) => upsertSymbol(db, symbol));
...
export async function getSymbol(db: Surreal, symbolId: string): Promise<SurrealSymbol | null> {
  try {
...
export async function findByFilePathAndHash(
  db: Surreal,
  filePath: string,
  contentHash: string
): Promise<SurrealSymbol | null> {
  try {
    const result = await db.query<[SurrealSymbol[]]>(
...
export async function createEdge(db: Surreal, from: string, to: string, type: EdgeType): Promise<void> {
  try {
    const fromRecordId = `symbol:\`${from}\``;
    const toRecordId = `symbol:\`${to}\``;

    await db.query(`RELATE ${fromRecordId}->${type}->${toRecordId}`);
  } catch (error) {
...
export async function batchCreateEdges(
  ...
  try {
    const promises = edges.map((edge) => createEdge(db, edge.from, edge.to, edge.type));
    await Promise.all(promises);
  } catch (error) {
...
export async function fullTextSearch(db: Surreal, options: FullTextSearchOptions): Promise<FullTextSearchResult[]> {
  try {
    const { query, workspaceId, limit = 10, filters } = options;

    let whereClause = `workspace_id = $workspace_id`;
    const params: Record<string, unknown> = { workspace_id: workspaceId, query, limit };
    ...
    const result = await db.query<[FullTextSearchResult[]]>(
      `
        SELECT
...
export async function queryGraph(db: Surreal, options: GraphQueryOptions): Promise<GraphQueryResult> {
  try {
    const { symbolId, edgeType, direction = 'outbound' } = options;
    const recordId = `symbol:\`${symbolId}\``;
    ...
    const edgesResult = await db.query<[Array<{ in: string; out: string }>]>(edgesQuery);
...
export async function deleteSymbol(db: Surreal, symbolId: string): Promise<void> {
  try {
    const recordId = `symbol:${symbolId}`;
    // @ts-ignore
...
export async function deleteByRepoId(db: Surreal, repoId: string): Promise<void> {
  try {
    await db.query('DELETE symbol WHERE repo_id = $repo_id', { repo_id: repoId });
...
export async function deleteByWorkspaceId(db: Surreal, workspaceId: string): Promise<void> {
  try {
    await db.query('DELETE symbol WHERE workspace_id = $workspace_id', {
...
export async function deleteVirtualSymbols(db: Surreal): Promise<number> {
  try {
    const result = await db.query<[{ count: number }[]]>('DELETE symbol WHERE workspace_id = "virtual" RETURN BEFORE');
