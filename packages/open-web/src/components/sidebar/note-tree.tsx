  ...
  return (
    <>
      <div className="space-y-0.5">
        {notes.map((item) => (
          <NoteItem key={item.id} item={item} level={0} inputDialog={inputDialog} confirmDialog={confirmDialog} />
        ))}
      </div>
...
function NoteItem({ item, level, inputDialog, confirmDialog }: NoteItemProps) {
  const { activeItemId, setActiveItem, isExpanded, toggleExpand, addNote, updateNote, deleteNote } = useSidebarStore();
  ...
  const handleAdd = async (type: string) => {
    ...
    if (name) {
      if (type === 'note') {
        addNote({ label: name, icon: 'FileText', type: 'note' }, item.id);
      } else if (type === 'folder') {
        addNote({ label: name, icon: 'Folder', type: 'note', children: [] }, item.id);
      }
    }
  };
  ...
  return (
    ...
    >
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className="opacity-0 transition-opacity group-hover:opacity-100" onClick={(e) => e.stopPropagation()}>
            <MoreVertical className="h-4 w-4 shrink-0 text-gray-400 hover:text-gray-600" />
          </button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-36">
