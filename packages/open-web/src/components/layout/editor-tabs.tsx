...
import { X, MessageSquare, FileText, File, ChevronDown } from 'lucide-react';
import { useTabsStore, type Tab } from '@/storage';
  ...
  const handleCloseTab = (e: React.MouseEvent, tabId: string) => {
    ...
    if (activeTabId === tabId && tabs.length > 1) {
      ...
      if (newActiveTab && newActiveTab.id !== tabId) {
        navigate({ to: getTabRoute(newActiveTab) });
      } else if (tabs[tabIndex + 1]) {
        navigate({ to: getTabRoute(tabs[tabIndex + 1]) });
      }
    }
  };
